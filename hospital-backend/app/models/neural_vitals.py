...
class VitalsRealtimeMessage(BaseModel):
    ...
    # Advanced vitals (from ESP32 sensor simulators v5.2.15+)
    bioimpedance: Optional[float] = Field(None, ge=20.0, le=50.0, description="Thoracic bioimpedance in Ohms (from MAX86178)")
    tremor: Optional[float] = Field(None, ge=0.0, le=10.0, description="Tremor intensity 0-10 scale (from BMI323)")
    imuFallRisk: Optional[float] = Field(None, ge=0.0, le=10.0, description="IMU-based fall risk 0-10 scale (from BMI323)")
    perfusionIndex: Optional[float] = Field(None, ge=0.0, le=20.0, description="Perfusion index percentage (from MAX86178)")
    stepCount: Optional[int] = Field(None, ge=0, description="Step count (from BMI323)")
    watchWorn: Optional[bool] = Field(None, description="Watch worn status (from MAX86178 proximity)")
    lastMovementTime: Optional[int] = Field(None, ge=0, description="Last movement timestamp in milliseconds (from BMI323)")
