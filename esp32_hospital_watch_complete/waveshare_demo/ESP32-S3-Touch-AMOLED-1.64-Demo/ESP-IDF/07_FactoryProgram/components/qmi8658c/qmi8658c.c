...
void qmi8658_read_Quaternion_data(float quatern[4])
{
	uint8_t buf[8] = {0};
	uint16_t w = 0,x = 0,y = 0,z = 0;
	if(qmi8658_read_reg(0x49,buf,8))
	{
		w = buf[0] | ((uint16_t)buf[1] << 8);
		x = buf[2] | ((uint16_t)buf[3] << 8);
		y = buf[4] | ((uint16_t)buf[5] << 8);
		z = buf[6] | ((uint16_t)buf[7] << 8);
		quatern[0] = w;
		quatern[1] = x;
		quatern[2] = y;
		quatern[3] = z;
