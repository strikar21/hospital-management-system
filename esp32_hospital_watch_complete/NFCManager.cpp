...
NFCManager::NFCManager(uint8_t sda_pin, uint8_t scl_pin, uint8_t irq_pin) {
    irqPin = irq_pin;
    sdaPin = sda_pin;
    sclPin = scl_pin;
    initialized = false;
    ...
    lastError = "";
    nfc = nullptr;
    i2cBus = new TwoWire(1);  // Create second I2C bus (I2C1)
...
bool NFCManager::begin() {
    Serial.println("[NFC] Initializing PN532 NFC module over I2C Bus 1...");
    Serial.println("[NFC] Using separate I2C bus: SDA=" + String(sdaPin) + ", SCL=" + String(sclPin));

    // Configure IRQ pin as input with pullup
    pinMode(irqPin, INPUT_PULLUP);

    // Initialize I2C Bus 1 on custom pins (separate from display touch I2C Bus 0)
    i2cBus->begin(sdaPin, sclPin, 400000);  // 400kHz

    // Create Adafruit PN532 with I2C interface using second bus
    nfc = new Adafruit_PN532(irqPin, -1, i2cBus);  // IRQ pin, no reset pin, custom I2C bus
